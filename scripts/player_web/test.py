...
if __name__ == "__main__":
    ...
    blue_hp_sub = node.create_subscription(
        Int32, "/referee_system/standard_robot_blue1/hp", log_func, 10
    )
    red_hp_sub = node.create_subscription(
        Int32, "/referee_system/standard_robot_red1/hp", log_func, 10
    )
    attack_info_sub = node.create_subscription(
        String, "/referee_system/attack_info", log_func, 10
    )
    rclpy.spin(node)
    node.destroy_node()
    # gimbal_pitch = 0.0
